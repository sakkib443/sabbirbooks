    ...
    return (
      <div className="space-y-4">
        ...
        <div className="bg-dash-card rounded-2xl border border-dash-line p-12 text-center">
          ...
          <h3 className="text-base font-bold text-dash-ink3">Batch not found</h3>
          <p className="text-sm text-dash-mute mt-1">You may not be enrolled in this batch.</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="relative rounded-2xl bg-dash-card border border-dash-line/60 shadow-sm overflow-hidden">
        ...
        <div className="absolute top-0 right-0 w-64 h-64 bg-gradient-to-br from-brand/10 via-brand-hover/5 to-transparent rounded-full blur-3xl pointer-events-none" />
        ...
        <div className="relative p-6 sm:p-7">
          <div className="flex items-start gap-5 flex-wrap">
            ...
            <div className="relative w-16 h-16 sm:w-20 sm:h-20 rounded-2xl bg-gradient-to-br from-brand via-brand-strong to-brand-hover flex items-center justify-center text-white shadow-lg shadow-brand/20 flex-shrink-0">
              <FiBookOpen size={28} />
            ...
            <div className="flex-1 min-w-0">
              ...
              <h1 className="text-xl sm:text-2xl font-bold text-dash-ink leading-tight">{course?.title || 'Course'}</h1>
              {batch?.name && (
                <p className="text-sm text-dash-mute mt-0.5">{batch.name}</p>
              )}
            </div>
          ...
          {/* Schedule + mentor row */}
          <div className="mt-5 pt-5 border-t border-dash-line-soft grid grid-cols-2 sm:grid-cols-4 gap-3">
            {batch?.classDays?.length > 0 && (
              <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-bold text-dash-mute2 uppercase tracking-wider">Class Days</p>
                  <p className="text-xs font-bold text-dash-ink3 truncate">{batch.classDays.map(d => d.slice(0, 3)).join(', ')}</p>
                </div>
              </div>
            )}
            {batch?.classTime && (
              <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-bold text-dash-mute2 uppercase tracking-wider">Class Time</p>
                  <p className="text-xs font-bold text-dash-ink3 truncate">{batch.classTime}</p>
                </div>
              </div>
            )}
            {mentor?.name && (
              <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-bold text-dash-mute2 uppercase tracking-wider">Mentor</p>
                  <p className="text-xs font-bold text-dash-ink3 truncate">{mentor.name}</p>
                </div>
              </div>
            )}
            {batch?.startDate && (
              <div className="flex items-center gap-2.5">
                ...
                <div className="min-w-0">
                  <p className="text-[10px] font-bold text-dash-mute2 uppercase tracking-wider">Duration</p>
                  <p className="text-xs font-bold text-dash-ink3 truncate">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <h2 className="text-lg font-bold text-dash-ink2 flex items-center gap-2">
          <FiFolder className="text-brand" size={18} />
          Class Folders <span className="text-sm text-dash-mute2 font-medium">({classes.length})</span>
        </h2>
        {classes.length > 0 && (
      ...
      {classes.length === 0 ? (
        <div className="bg-dash-card rounded-2xl border border-dashed border-dash-line p-16 text-center">
          <div className="w-16 h-16 rounded-full bg-dash-soft flex items-center justify-center mx-auto mb-3">
            <FiFolder className="text-dash-faint" size={24} />
          </div>
          <h3 className="text-base font-bold text-dash-ink3">No class folders yet</h3>
        ...
        <div className="bg-dash-card rounded-2xl border border-dash-line p-8 text-center">
          <p className="text-sm text-dash-mute">No class matches your search.</p>
        </div>
      ) : (
          ...
          {filtered.map((cls, idx) => {
            ...
            return (
              <Link
                ...
                className="group relative bg-dash-card rounded-2xl border border-dash-line/60 shadow-sm hover:shadow-xl hover:-translate-y-1 hover:border-brand-line transition-all duration-300 overflow-hidden"
              >
                {/* Top color band */}
                <div className={`h-2 ${
                  isCompleted ? 'bg-gradient-to-r from-brand to-brand-hover'
                ...
                <div className="p-4">
                  ...
                  {/* Title */}
                  <h3 className="font-bold text-dash-ink2 text-sm leading-snug line-clamp-2 group-hover:text-brand-deep transition mb-1 min-h-[2.5rem]">
                    {cls.title || `Class ${idx + 1}`}
                  </h3>

                  {/* Topic */}
                  {cls.topic && (
                    <p className="text-[11px] text-dash-mute line-clamp-2 mb-3 min-h-[2rem]">{cls.topic}</p>
                  )}

                  {/* Date + time */}
                  <div className="flex items-center gap-2 text-[11px] text-dash-mute mb-3">
                    {classDate && (
                  ...
                  {/* Content badges */}
                  <div className="flex items-center gap-1.5 flex-wrap pt-3 border-t border-dash-line-soft">
                    {recs > 0 && (
                    ...
                    {cls.venue && (
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-dash-soft2 text-dash-ink4 text-[10px] font-bold">
                        <FiMapPin size={9} /> venue
                      </span>
                    )}
                    {recs === 0 && mats === 0 && !cls.meetingLink && !cls.venue && (
                      <span className="text-[10px] text-dash-mute2 italic">No content yet</span>
                    )}
                  </div>
                </div>
              </Link>
