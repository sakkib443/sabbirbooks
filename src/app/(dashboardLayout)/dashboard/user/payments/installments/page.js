...
export default function InstallmentsPage() {
  ...
  const getStatusStyle = (status) => {
    switch (status) {
      ...
      default: return { bg: 'bg-slate-50', text: 'text-slate-500', border: 'border-slate-200', icon: FiClock };
    }
  };
  ...
  if (loading) {
    return (
      <div className="p-6 min-h-screen bg-slate-50 flex items-center justify-center">
        ...
        <p className="ml-3 text-slate-500">Loading installments...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 lg:p-8 min-h-screen bg-slate-50">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-900 outfit">Installment Tracker</h1>
        <p className="text-slate-500 mt-1">Track your installment payments</p>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-5 shadow-sm">
          <div className="flex items-center gap-3">
            <div className={`w-11 h-11 rounded-xl flex items-center justify-center ${overdueCount > 0 ? 'bg-red-50' : 'bg-slate-50'}`}>
              <FiAlertTriangle className={overdueCount > 0 ? 'text-red-600' : 'text-slate-400'} size={20} />
            </div>
            <div>
              <p className="text-xs text-slate-500">Overdue</p>
              <p className={`text-xl font-bold outfit ${overdueCount > 0 ? 'text-red-600' : 'text-slate-900'}`}>{overdueCount}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      {Object.keys(grouped).length === 0 ? (
        <div className="bg-white rounded-2xl border border-slate-200 p-16 text-center">
          ...
          <h3 className="text-lg font-bold text-slate-700 mb-2">No Installments</h3>
          <p className="text-slate-500 text-sm">You don&apos;t have any installment plans.</p>
        </div>
      ) : (
        <div className="space-y-6">
          {groups.map((g) => {
            ...
            return (
            <div key={course?._id || 'unknown'} className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
              {/* Course Header */}
              <div className="px-6 py-4 bg-gradient-to-r from-slate-50 to-white border-b border-slate-100">
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <h3 className="font-bold text-slate-800 truncate">{course?.title || 'Course'}</h3>
                    <p className="text-xs text-slate-500">{items.length} installments</p>
                  </div>
                </div>
                {/* Money summary (Model A) */}
                <div className="grid grid-cols-4 gap-2 mt-3">
                  {[
                    { label: 'Total Fee', value: m.agreedTotal, color: 'text-slate-700' },
                    ...
                    <div key={c.label} className="rounded-lg bg-slate-50 border border-slate-100 px-2 py-1.5 text-center">
                    ...
                    return (
                      <div key={inst._id} className="flex gap-4">
                        ...
                        <div className={`flex-1 flex items-center justify-between p-4 rounded-xl border ${style.border} ${style.bg} mb-1`}>
                          ...
                          <div className="flex items-center gap-3">
                            <span className="text-lg font-bold text-slate-800">৳{inst.amount.toLocaleString()}</span>
                            {(inst.status === 'due' || inst.status === 'overdue') && (
                              <button
                                onClick={() => handlePayInstallment(inst._id)}
                                disabled={payingId === inst._id}
                                className="px-4 py-2 bg-gradient-to-r from-[#F3A522] to-[#d88f13] text-white text-xs font-bold rounded-lg hover:shadow-lg disabled:opacity-50 transition-all"
                              >
