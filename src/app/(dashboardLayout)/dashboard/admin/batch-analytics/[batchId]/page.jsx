...
const MiniProgress = ({ value, color = 'teal', label, sublabel }) => (
  <div className="flex-1">
    <div className="flex justify-between items-center mb-1">
      <span className="text-[11px] text-dash-mute font-medium">{label}</span>
      <span className={`text-[11px] font-bold text-${color}-600`}>{value}%</span>
    </div>
    <div className="w-full bg-dash-soft2 rounded-full h-1.5">
      <div className={`h-1.5 rounded-full bg-${color}-500 transition-all duration-500`} style={{ width: `${Math.min(value, 100)}%` }} />
    </div>
    {sublabel && <p className="text-[10px] text-dash-mute2 mt-0.5">{sublabel}</p>}
  </div>
);

// ─── Stat Mini Card ─────────────────────────────────────────
const StatMini = ({ icon: Icon, label, value, color = 'slate', sub }) => (
  <div className="bg-dash-card rounded-xl border border-dash-line-soft p-3 flex items-center gap-3">
    <div className={`w-9 h-9 rounded-lg bg-${color}-50 flex items-center justify-center flex-shrink-0`}>
      <Icon size={16} className={`text-${color}-500`} />
    </div>
    <div>
      <p className="text-lg font-bold text-dash-ink2 leading-none">{value}</p>
      <p className="text-[10px] text-dash-mute2 font-medium mt-0.5">{label}</p>
      {sub && <p className="text-[10px] text-dash-mute2">{sub}</p>}
    </div>
  </div>
);
...
function StudentSection({ students, onRefresh }) {
  ...
  if (students.length === 0) {
    return <div className="text-center py-8 text-sm text-dash-mute2"><FiUsers className="mx-auto mb-2" size={28} /> No students enrolled yet</div>;
  }
  ...
  return (
    <div className="bg-dash-card rounded-xl border border-dash-line-soft overflow-x-auto">
      <table className="w-full">
        <thead>
          <tr className="bg-dash-soft border-b border-dash-line-soft">
            <th className="text-left px-3 py-3 text-[13px] font-semibold text-dash-ink4">Name</th>
            <th className="text-left px-3 py-3 text-[13px] font-semibold text-dash-ink4">Email</th>
            <th className="text-center px-3 py-3 text-[13px] font-semibold text-dash-ink4">Phone</th>
            <th className="text-center px-3 py-3 text-[13px] font-semibold text-dash-ink4">Status</th>
            <th className="text-center px-3 py-3 text-[13px] font-semibold text-dash-ink4">Attendance</th>
            <th className="text-center px-3 py-3 text-[13px] font-semibold text-dash-ink4">Exams</th>
          </tr>
        </thead>
        <tbody>
          {students.map((s, i) => (
            <tr key={s._id || i} className="border-b border-dash-soft last:border-0 hover:bg-dash-soft/50">
              <td className="px-3 py-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="text-sm font-semibold text-dash-ink2 truncate max-w-[160px]">{s.name}</span>
                </div>
              </td>
              <td className="px-3 py-3"><span className="text-sm text-dash-mute truncate block max-w-[180px]">{s.email || ''}</span></td>
              <td className="px-3 py-3 text-center"><span className="text-sm text-dash-ink4">{s.phone || '—'}</span></td>
              ...
              <td className="px-3 py-3 text-center">
                <div className="flex flex-col items-center gap-1">
                  <div className="flex items-center gap-2">
                    <div className="w-16 bg-dash-soft2 rounded-full h-2">
                  ...
                  <span className="text-[11px] text-dash-mute2">{s.attendancePresent}/{s.attendanceSessions || s.attendanceTotal} classes</span>
                </div>
              </td>
              <td className="px-3 py-3 text-center">
                <span className={`text-sm font-semibold ${s.assignmentsTotal > 0 && s.assignmentsSubmitted >= s.assignmentsTotal ? 'text-emerald-600' : s.assignmentsSubmitted > 0 ? 'text-amber-600' : 'text-dash-mute2'}`}>{s.assignmentsSubmitted || 0}/{s.assignmentsTotal || 0}</span>
                <p className="text-[11px] text-dash-mute2">submitted</p>
              </td>
            </tr>
          ))}
        </tbody>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-dash-card rounded-xl border border-dash-line-soft overflow-hidden">
        <div className="flex items-center justify-between px-4 py-3 bg-dash-soft border-b border-dash-line-soft">
          <h4 className="text-xs font-semibold text-dash-ink3">💰 Student Payment Details</h4>
          {coursePrice > 0 && <span className="text-[10px] text-dash-mute2">Course: {formatTaka(coursePrice)}</span>}
        </div>
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-dash-line-soft">
              <th className="text-left px-3 py-3 text-[13px] font-semibold text-dash-ink4">Student</th>
              <th className="text-center px-3 py-3 text-[13px] font-semibold text-dash-ink4">Phone</th>
              <th className="text-center px-3 py-3 text-[13px] font-semibold text-dash-ink4">Paid</th>
              <th className="text-center px-3 py-3 text-[13px] font-semibold text-dash-ink4">Due</th>
              <th className="text-center px-3 py-3 text-[13px] font-semibold text-dash-ink4">Installments</th>
              <th className="text-center px-3 py-3 text-[13px] font-semibold text-dash-ink4">Action</th>
            </tr>
          </thead>
          <tbody>
            {perStudent.map((ps, i) => (
              <React.Fragment key={ps.studentId || i}>
                <tr className="border-b border-dash-soft last:border-0 hover:bg-dash-soft/30">
                  <td className="px-3 py-3"><p className="text-sm font-semibold text-dash-ink3 truncate max-w-[160px]">{ps.name}</p><p className="text-xs text-dash-mute2 truncate">{ps.email}</p></td>
                  <td className="px-3 py-3 text-center"><span className="text-sm text-dash-ink4">{ps.phone || '—'}</span></td>
                  <td className="px-3 py-3 text-center"><span className="text-sm font-bold text-emerald-600">{formatTaka(ps.totalPaid)}</span></td>
                  <td className="px-3 py-3 text-center"><span className={`text-sm font-bold ${ps.totalDue > 0 ? 'text-red-500' : 'text-dash-mute2'}`}>{ps.totalDue > 0 ? formatTaka(ps.totalDue) : '—'}</span></td>
                  <td className="px-3 py-3 text-center">
                ...
                {expandedStudent === ps.studentId && ps.installments?.length > 0 && (
                  <tr><td colSpan={6} className="p-0">
                    <div className="bg-dash-soft/80 px-4 py-3 border-b border-dash-line-soft">
                      <p className="text-xs font-semibold text-dash-ink4 mb-2">📋 Installment History — {ps.name}</p>
                      <div className="space-y-2">
                        {ps.installments.map((inst, idx) => (
                          <div key={inst._id || idx} className="flex items-center justify-between bg-dash-card rounded-lg px-4 py-2.5 border border-dash-line-soft">
                            <div className="flex items-center gap-3">
                              ...
                              <div><p className="text-sm font-semibold text-dash-ink3">{formatTaka(inst.amount)}</p><p className="text-xs text-dash-mute2">{inst.notes || ''}</p></div>
                            </div>
                            <div className="text-right">
                              ...
                              <p className="text-xs text-dash-mute2 mt-0.5">{inst.status === 'paid' && inst.paidDate ? formatDate(inst.paidDate) : formatDate(inst.dueDate)}</p>
                            </div>
                          </div>
                        ))}
                      </div>
      ...
      {showAddModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setShowAddModal(null)}>
          <div className="bg-dash-card rounded-2xl shadow-xl w-full max-w-md p-6" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              <div><h3 className="text-base font-bold text-dash-ink2">Add Installment</h3><p className="text-xs text-dash-mute2">{showAddModal.name}</p></div>
              <button onClick={() => setShowAddModal(null)} className="p-1.5 hover:bg-dash-soft2 rounded-lg"><FiX size={16} /></button>
            </div>
            <div className="space-y-3">
              <div><label className="text-[11px] font-semibold text-dash-ink4 mb-1 block">Amount (৳) *</label><input type="number" value={addForm.amount} onChange={(e) => setAddForm({ ...addForm, amount: e.target.value })} className="w-full px-3 py-2 text-sm border border-dash-line rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500/20 focus:border-teal-400" placeholder="e.g. 5000" /></div>
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-[11px] font-semibold text-dash-ink4 mb-1 block">Status</label><select value={addForm.status} onChange={(e) => setAddForm({ ...addForm, status: e.target.value })} className="w-full px-3 py-2 text-sm border border-dash-line rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500/20"><option value="paid">Paid</option><option value="due">Due</option><option value="upcoming">Upcoming</option></select></div>
                <div><label className="text-[11px] font-semibold text-dash-ink4 mb-1 block">Method</label><select value={addForm.method} onChange={(e) => setAddForm({ ...addForm, method: e.target.value })} className="w-full px-3 py-2 text-sm border border-dash-line rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500/20"><option value="manual">Manual</option><option value="bkash">bKash</option><option value="sslcommerz">SSLCommerz</option></select></div>
              </div>
              <div><label className="text-[11px] font-semibold text-dash-ink4 mb-1 block">Notes</label><input type="text" value={addForm.notes} onChange={(e) => setAddForm({ ...addForm, notes: e.target.value })} className="w-full px-3 py-2 text-sm border border-dash-line rounded-lg focus:outline-none focus:ring-2 focus:ring-teal-500/20" placeholder="Optional notes" /></div>
              <div className="bg-dash-soft rounded-lg p-3 text-[10px] text-dash-mute">
                <p>Course Price: <strong className="text-dash-ink3">{formatTaka(showAddModal.coursePrice || coursePrice)}</strong></p>
...
function AttendanceSection({ students, details }) {
  ...
  const statusCell = (status) => {
    ...
    return <span className="text-[9px] text-dash-faint">—</span>;
  };
  if (students.length === 0) return <div className="text-center py-8 text-sm text-dash-mute2"><FiUsers className="mx-auto mb-2" size={28} /> No students enrolled yet</div>;
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-2">
        <div className="flex items-center gap-3">
          <h4 className="text-xs font-semibold text-dash-ink3">📋 Attendance Grid</h4>
        ...
        <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="text-xs text-dash-ink4 bg-dash-card border border-dash-line rounded-lg px-2.5 py-1.5 outline-none">
      ...
      {grid.length === 0 ? (
        <div className="bg-dash-card border border-dashed border-dash-line-strong rounded-xl p-8 text-center">
          <FiBook className="mx-auto text-dash-faint mb-2" size={28} />
          <p className="text-sm text-dash-mute">No attendance records yet</p>
          <p className="text-xs text-dash-mute2 mt-1">Attendance data will appear here once the mentor starts taking attendance</p>
        </div>
      ) : (
            ...
            <table className="w-full border-collapse" style={{ minWidth: `${260 + grid.length * 60 + 200}px` }}>
              <thead>
                <tr className="bg-dash-soft border-b border-dash-line">
                  <th className="sticky left-0 z-10 bg-dash-soft text-left px-3 py-2.5 text-[10px] font-bold text-dash-mute border-r border-dash-line w-8">#</th>
                  <th className="sticky left-8 z-10 bg-dash-soft text-left px-3 py-2.5 text-[10px] font-bold text-dash-mute border-r border-dash-line min-w-[180px]">Student</th>
                  {grid.map((session, idx) => (
                    <th key={session._id || idx} className="text-center px-1 py-2.5 text-[9px] font-bold text-dash-mute min-w-[52px] border-r border-dash-line-soft last:border-r-0" title={session.classTitle || fmtCol(session.date)}>
                      <div>{fmtCol(session.date)}</div>
                      {session.classTitle && <div className="text-[8px] font-normal text-dash-mute2 truncate max-w-[50px]">{session.classTitle}</div>}
                    </th>
                  ))}
                  ...
                  <th className="text-center px-2 py-2.5 text-[10px] font-bold text-dash-ink4 bg-dash-soft2 min-w-[55px]">%</th>
                  <th className="text-center px-2 py-2.5 text-[10px] font-bold text-dash-ink4 bg-dash-soft2 min-w-[60px]">Status</th>
                </tr>
              </thead>
              <tbody>
                {sorted.map((s, i) => {
                  ...
                  return (
                    <tr key={s._id || i} className={`border-b border-dash-soft last:border-0 ${isAtRisk ? 'bg-red-50/20' : i % 2 === 0 ? 'bg-dash-card' : 'bg-dash-soft/30'}`}>
                      <td className="sticky left-0 z-10 bg-inherit px-3 py-2 text-[10px] text-dash-mute2 border-r border-dash-line">{i + 1}</td>
                      <td className="sticky left-8 z-10 bg-inherit px-3 py-2 border-r border-dash-line">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[11px] font-semibold text-dash-ink2 truncate max-w-[140px]">{s.name}</span>
                        </div>
                      </td>
                      {grid.map((session, idx) => (
                        <td key={session._id || idx} className="text-center px-1 py-2 border-r border-dash-soft">{statusCell(getStatus(session.records, s._id))}</td>
                      ))}
                      ...
                      <td className="text-center px-2 py-2 bg-amber-50/30"><span className={`text-[11px] font-bold ${late > 0 ? 'text-amber-600' : 'text-dash-faint'}`}>{late}</span></td>
                      <td className="text-center px-2 py-2 bg-red-50/30"><span className={`text-[11px] font-bold ${absent > 0 ? 'text-red-600' : 'text-dash-faint'}`}>{absent}</span></td>
                      <td className="text-center px-2 py-2 bg-dash-soft/50"><span className={`text-[11px] font-bold ${pct >= 90 ? 'text-emerald-600' : pct >= 75 ? 'text-teal-600' : pct >= 60 ? 'text-amber-600' : 'text-red-500'}`}>{pct}%</span></td>
                      <td className="text-center px-2 py-2 bg-dash-soft/50">
                        {total === 0 ? <span className="text-[9px] font-semibold px-1.5 py-0.5 rounded-full bg-dash-soft2 text-dash-mute2">—</span>
...
export default function BatchDetailPage() {
  ...
  if (!details) {
    return <div className="p-10 text-center text-sm text-dash-mute2">Failed to load batch details. <Link href="/dashboard/admin/batch-analytics" className="text-violet-600 underline">Go back</Link></div>;
  }
  ...
  const statusBadge = batchStatus === 'running' ? 'bg-emerald-50 text-emerald-600' : batchStatus === 'upcoming' ? 'bg-blue-50 text-blue-600' : 'bg-dash-soft2 text-dash-mute';
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-3">
        <div className="flex items-center gap-3">
          <Link href="/dashboard/admin/batch-analytics" className="w-9 h-9 rounded-lg bg-dash-card border border-dash-line flex items-center justify-center text-dash-mute hover:text-violet-600 hover:border-violet-200 transition shrink-0">
            <FiArrowLeft size={16} />
          </Link>
          <div className="w-12 h-12 rounded-xl overflow-hidden bg-dash-soft2 shrink-0">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-lg font-bold text-dash-ink outfit">{batch.name || batch.courseName || 'Batch'}</h1>
              <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${statusBadge}`}>{batchStatus}</span>
            </div>
            <p className="text-xs text-dash-mute2 mt-0.5 flex flex-wrap items-center gap-3">
      ...
      <div className="bg-dash-card rounded-xl border border-dash-line/60 shadow-sm">
        <div className="flex items-center gap-2 px-4 pt-4 pb-0 overflow-x-auto border-b border-dash-line-soft">
          {sections.map((s) => (
            <button key={s} onClick={() => setActiveSection(s)}
              className={`px-4 py-2.5 rounded-t-lg text-sm font-semibold whitespace-nowrap transition border-b-2 -mb-px ${activeSec === s ? 'border-violet-500 text-violet-600' : 'border-transparent text-dash-mute hover:text-dash-ink3 hover:bg-dash-soft'}`}>
              {sectionLabels[s]}
            </button>
          ))}
        </div>
        ...
        <div className="p-4">
            ...
            <div className="space-y-4">
              {batchStatus === 'running' && (
                <>
                  ...
                  <div className="bg-dash-card rounded-xl border border-dash-line-soft p-3">
                    <div className="flex flex-wrap gap-4 text-[11px] text-dash-mute">
                      {batch.classTime && <span>🕐 Class Time: <strong className="text-dash-ink3">{batch.classTime}</strong></span>}
                      {batch.classDays?.length > 0 && <span>📅 Days: <strong className="text-dash-ink3">{batch.classDays.join(', ')}</strong></span>}
                      {batch.mentorId?.name && <span>👨‍🏫 Mentor: <strong className="text-dash-ink3">{batch.mentorId.name}</strong></span>}
                    </div>
                  </div>
                </>
              )}
                ...
                <>
                  ...
                  <div className="bg-dash-card rounded-xl border border-dash-line-soft p-3">
                    <div className="flex flex-wrap gap-4 text-[11px] text-dash-mute">
                      ...
                      {batch.classDays?.length > 0 && <span>📅 Days: <strong className="text-dash-ink3">{batch.classDays.join(', ')}</strong></span>}
                      {batch.mentorId?.name && <span>👨‍🏫 Mentor: <strong className="text-dash-ink3">{batch.mentorId.name}</strong></span>}
                      <span>💺 Max Seats: <strong className="text-dash-ink3">{batch.maxStudents || 50}</strong></span>
                    </div>
                  </div>
                </>
              )}
          ...
          {activeSec === 'certificates' && (
            <div className="space-y-4">
              ...
              {certificates.total === 0 && <div className="text-center py-4 text-sm text-dash-mute2"><FiAward className="mx-auto mb-2" size={24} /> No certificates issued yet</div>}
              {students.length > 0 && (
                <div className="bg-dash-card rounded-xl border border-dash-line-soft p-3">
