  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-dash-ink outfit">Reports</h1>
          <p className="text-sm text-dash-mute mt-1">Download & analyze your academy data</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={card.type} className="bg-dash-card rounded-xl border border-dash-line/60 shadow-sm overflow-hidden group hover:shadow-md transition">
            <div className={`h-1.5 bg-gradient-to-r ${card.color}`} />
            <div className="p-5">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-sm font-medium text-dash-ink3">{card.label}</p>
                  <p className="text-xs text-dash-mute2 mt-0.5">{card.desc}</p>
                </div>
              ...
              <button
                ...
                className="mt-4 w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-dash-soft hover:bg-dash-soft2 text-sm text-dash-ink4 font-medium transition border border-dash-line"
              >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Monthly Enrollment Trend */}
        <div className="lg:col-span-2 bg-dash-card rounded-xl border border-dash-line/60 shadow-sm p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-base font-semibold text-dash-ink3 outfit-semibold">Monthly Enrollments</h3>
              <p className="text-xs text-dash-mute2 mt-0.5">Last 6 months trend</p>
            </div>
          ...
          <div className="flex items-end gap-3 h-48">
            {monthlyData.map((d, i) => (
              <div key={i} className="flex-1 flex flex-col items-center gap-2">
                <span className="text-xs font-medium text-dash-ink4">{d.count}</span>
                <div
                ...
                />
                <span className="text-xs text-dash-mute2">{d.month}</span>
              </div>
            ))}
            {monthlyData.length === 0 && (
              <div className="flex-1 flex items-center justify-center text-sm text-dash-mute2">No data yet</div>
            )}
          </div>
        </div>
        ...
        <div className="bg-dash-card rounded-xl border border-dash-line/60 shadow-sm p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-base font-semibold text-dash-ink3 outfit-semibold">Status Breakdown</h3>
              <p className="text-xs text-dash-mute2 mt-0.5">Order distribution</p>
            </div>
              ...
              return (
                <div key={status}>
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-sm text-dash-ink4 capitalize">{status}</span>
                    <span className="text-sm font-medium text-dash-ink3">{count} ({pct}%)</span>
                  </div>
                  <div className="w-full bg-dash-soft2 rounded-full h-2">
                    <div
                      className={`h-2 rounded-full transition-all ${statusColors[status] || 'bg-dash-mute2'}`}
                      style={{ width: `${pct}%` }}
                    />
                  </div>
                </div>
          ...
          <div className="mt-6 pt-6 border-t border-dash-line-soft">
            <p className="text-sm font-medium text-dash-mute mb-3">By Course Type</p>
            <div className="space-y-2">
              {Object.entries(courseTypeMap).map(([type, count]) => (
                <div key={type} className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <FiGlobe size={12} className="text-dash-mute2" />
                    <span className="text-sm text-dash-ink4 capitalize">{type}</span>
                  </div>
                  <span className="text-sm font-medium text-dash-ink3">{count}</span>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="bg-dash-card rounded-xl border border-dash-line/60 shadow-sm p-6">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h3 className="text-base font-semibold text-dash-ink3 outfit-semibold">Top Courses by Enrollment</h3>
            <p className="text-xs text-dash-mute2 mt-0.5">Most popular courses</p>
          </div>
        ...
        <div className="space-y-4">
          {topCourses.map(([title, count], i) => (
            <div key={title} className="flex items-center gap-4">
              <span className={`w-7 h-7 rounded-lg flex items-center justify-center text-xs font-bold ${
                i === 0 ? 'bg-amber-100 text-amber-600' : i === 1 ? 'bg-dash-soft3 text-dash-ink4' : 'bg-orange-50 text-orange-500'
              }`}>{i + 1}</span>
              <div className="flex-1 min-w-0">
                <p className="text-sm text-dash-ink3 truncate">{title}</p>
                <div className="w-full bg-dash-soft2 rounded-full h-2 mt-1">
                  <div
          ...
          {topCourses.length === 0 && (
            <p className="text-sm text-dash-mute2 text-center py-8">No enrollment data available</p>
          )}
        </div>
      </div>
    </div>
