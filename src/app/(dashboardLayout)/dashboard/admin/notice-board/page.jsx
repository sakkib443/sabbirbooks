...
export default function AdminNoticeBoardPage() {
  ...
  const AudBadge = ({ a }) => a === 'enrolled'
    ? <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-black uppercase bg-[#FEF6E7] text-[#a5680f]"><FiUsers size={10} /> Enrolled</span>
  ...
  const STAT_CARDS = [
    { label: 'Total Notices', value: stats.total, icon: FiClipboard, color: 'text-slate-700', bg: 'bg-slate-100' },
    { label: 'Active', value: stats.active, icon: FiEye, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Public', value: stats.public, icon: FiGlobe, color: 'text-[#c9871a]', bg: 'bg-[#FEF6E7]' },
    { label: 'Enrolled only', value: stats.enrolled, icon: FiUsers, color: 'text-slate-500', bg: 'bg-slate-50' },
  ];
    ...
    <div className="space-y-6 poppins">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">নোটিশ প্রকাশ করুন — সবার জন্য (Public) অথবা শুধু ভর্তিকৃত স্টুডেন্টদের জন্য।</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-2xl border border-dashed border-slate-200 p-16 text-center">
          ...
          <h3 className="text-base font-bold text-slate-700">{list.length === 0 ? 'No notices yet' : 'No notices match'}</h3>
          <p className="text-sm text-slate-400 mt-1">{list.length === 0 ? 'প্রথম নোটিশটি প্রকাশ করুন।' : 'অন্য search/filter দিন।'}</p>
          {list.length === 0 && (
            <button onClick={openNew} className="mt-5 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-[#F3A522] text-white text-sm font-bold"><FiPlus size={14} /> New Notice</button>
          )}
        </div>
      ) : (
        <div className="space-y-3">
          {filtered.map((n) => (
            <div key={n._id} className={`bg-white rounded-xl border shadow-sm p-4 flex items-start gap-4 ${n.isActive === false ? 'border-slate-200 opacity-70' : 'border-slate-200'}`}>
              <div className="w-11 h-11 rounded-xl bg-[#FEF6E7] flex items-center justify-center shrink-0">
                {n.attachmentType === 'image' ? <FiImage className="text-[#c9871a]" size={18} /> : n.attachmentType === 'pdf' ? <FiFileText className="text-[#c9871a]" size={18} /> : <FiClipboard className="text-[#c9871a]" size={18} />}
              </div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <h3 className="font-bold text-slate-800 truncate">{n.title}</h3>
                  <AudBadge a={n.audience} />
                  {n.isActive === false && <span className="px-2 py-0.5 rounded-full text-[9px] font-black uppercase bg-slate-100 text-slate-400">Hidden</span>}
                </div>
                {n.body && <p className="text-sm text-slate-500 mt-1 line-clamp-2">{n.body}</p>}
                <div className="flex items-center gap-3 mt-2 text-[11px] text-slate-400">
                  <span>{fmtDate(n.createdAt)}</span>
                  {n.attachmentUrl && (
                    <a href={n.attachmentUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[#c9871a] hover:underline">
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button onClick={() => toggleActive(n)} title={n.isActive === false ? 'Show' : 'Hide'} className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-[#c9871a] hover:bg-[#FEF6E7] transition">
                  {n.isActive === false ? <FiEyeOff size={15} /> : <FiEye size={15} />}
                </button>
                <button onClick={() => openEdit(n)} title="Edit" className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-[#c9871a] hover:bg-[#FEF6E7] transition"><FiEdit3 size={15} /></button>
                <button onClick={() => del(n)} title="Delete" className="w-8 h-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-rose-500 hover:bg-rose-50 transition"><FiTrash2 size={15} /></button>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    <button key={o.v} type="button" onClick={() => F('audience', o.v)}
                      className={`flex items-start gap-2 p-3 rounded-lg border-2 text-left transition ${form.audience === o.v ? 'border-[#F3A522] bg-[#FEF6E7]/50' : 'border-slate-200 hover:border-slate-300'}`}>
                      <o.icon size={16} className={form.audience === o.v ? 'text-[#c9871a] mt-0.5' : 'text-slate-400 mt-0.5'} />
                      <div><p className="text-sm font-bold text-slate-800">{o.t}</p><p className="text-[10px] text-slate-400">{o.d}</p></div>
                    </button>
                  ))}
                </div>
              </div>
              ...
              <div>
                ...
                {form.attachmentUrl ? (
                  <div className="flex items-center justify-between rounded-lg border border-[#F0DFB4] bg-[#FEF6E7]/50 px-3 py-2.5">
                    <a href={form.attachmentUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 min-w-0 text-sm text-[#a5680f] hover:underline">
                    ...
                    <button onClick={removeAttachment} className="text-slate-400 hover:text-rose-500 shrink-0"><FiX size={16} /></button>
                  </div>
                ) : (
              ...
              <label className="flex items-center gap-2 text-sm text-slate-600 cursor-pointer">
                <input type="checkbox" checked={form.isActive} onChange={(e) => F('isActive', e.target.checked)} className="accent-[#F3A522]" /> Active (এখনই দেখাবে)
              </label>
            </div>
