...
export default function MentorsPage() {
  ...
  const handleDelete = async (id) => {
    const ok = await confirm({ title: 'Delete this mentor?', message: 'The mentor and their login access will be removed. This cannot be undone.', confirmText: 'Delete', danger: true });
    if (!ok) return;
    try {
      const res = await fetch(`${API}/mentors/${id}`, { method: 'DELETE', headers: authHeaders() });
      const data = await res.json().catch(() => ({}));
      if (res.ok && data.success !== false) {
        setMentors((prev) => prev.filter((m) => m._id !== id));
        showToast('success', 'Mentor deleted');
      } else {
        showToast('error', data.message || 'Failed to delete mentor');
      }
    } catch (error) {
      showToast('error', 'Failed to delete mentor');
    }
  };

  const handleToggleVisibility = async (mentor) => {
    const next = mentor.isPublished === false; // toggling → show if currently hidden
    try {
      const res = await fetch(`${API}/mentors/${mentor._id}`, {
        method: 'PATCH', headers: authHeaders(), body: JSON.stringify({ isPublished: next }),
      });
      const data = await res.json();
      if (res.ok && data.success !== false) {
        setMentors((prev) => prev.map((m) => m._id === mentor._id ? { ...m, isPublished: next } : m));
        showToast('success', next ? 'Now shown on website' : 'Hidden from website');
      } else {
            ...
            <div key={mentor._id} className="group bg-white rounded-xl border border-slate-200 overflow-hidden hover:shadow-lg transition-all duration-300">
              ...
              <div className="relative h-48 overflow-hidden">
                ...
                <button
                  ...
                  title={mentor.isPublished === false ? 'Hidden from website — click to show' : 'Shown on website — click to hide'}
                  className="absolute top-2 right-2 w-8 h-8 rounded-lg bg-black/40 backdrop-blur text-white flex items-center justify-center hover:bg-black/60 transition"
                >
                  {mentor.isPublished === false ? <FiEyeOff size={14} /> : <FiEye size={14} />}
                </button>
                {mentor.isPublished === false && (
                  <span className="absolute top-2 left-2 px-2 py-0.5 rounded bg-gray-900/75 text-white text-[10px] font-bold uppercase tracking-wide">Hidden</span>
                )}
