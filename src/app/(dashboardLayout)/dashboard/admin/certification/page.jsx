...
export default function CertificationPage() {
  ...
  const getStatusBadge = (status) => {
    ...
    return { label: 'Completed', cls: 'bg-dash-soft2 text-dash-mute border-dash-line' };
  };
      ...
      <div className="poppins space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-dash-card rounded-xl border border-dash-line/60 p-4 shadow-sm">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-dash-ink outfit">Certification Management</h1>
              <p className="text-xs text-dash-mute2">Manage student certificates by batch</p>
            </div>
          </div>
          <button onClick={loadBatches} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-dash-ink4 bg-dash-soft border border-dash-line rounded-lg hover:bg-dash-soft2 transition">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <div className="flex items-center gap-2">
              <div className="relative">
                <FiBook size={13} className="absolute left-2.5 top-[8px] pointer-events-none text-dash-mute2" />
                <select value={courseFilter} onChange={e => setCourseFilter(e.target.value)}
                  className={`appearance-none pl-8 pr-7 py-1.5 rounded-lg border text-sm cursor-pointer outline-none transition max-w-[220px] truncate ${courseFilter !== 'all' ? 'bg-teal-50 text-teal-600 border-teal-200' : 'bg-dash-card text-dash-ink4 border-dash-line'}`}>
              ...
              {[
                ...
                return (
                  <button key={t.key} onClick={() => setTab(t.key)}
                    className={`flex items-center gap-2 px-4 py-2.5 rounded-t-lg text-xs font-medium transition-all border-b-2 ${isActive ? `border-${t.color}-500 text-${t.color}-600 bg-${t.color}-50/50` : 'border-transparent text-dash-mute hover:text-dash-ink3 hover:bg-dash-soft'}`}>
                    <Icon size={13} />
                    {t.label}
                    <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${isActive ? `bg-${t.color}-100 text-${t.color}-600` : 'bg-dash-soft2 text-dash-mute'}`}>{t.count}</span>
                  </button>
                );
              })}
            </div>
            {hasActiveFilter && (
              <span className="text-[10px] text-dash-mute2 font-medium">
                Showing {currentBatches.length} of {allCurrentBatches.length}
              </span>
            )}
          </div>

          <div className="border-t border-dash-line-soft" />

          <div className="p-4">
            {loading ? (
              <div className="space-y-3">
                {[1, 2, 3].map(i => (
                  <div key={i} className="h-[80px] bg-dash-soft rounded-xl animate-pulse" />
                ))}
              </div>
            ) : currentBatches.length === 0 ? (
              <div className="text-center py-12">
                <FiBook size={32} className="text-dash-faint mx-auto mb-3" />
                ...
                {currentBatches.map((batch) => {
                  ...
                  return (
                    <div key={batch._id} onClick={() => openBatch(batch)}
                      className="bg-dash-card border border-dash-line/60 rounded-xl p-4 cursor-pointer hover:shadow-md hover:border-teal-200 transition-all group">
                      <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                        <div className="w-14 h-14 rounded-xl overflow-hidden bg-dash-soft2 shrink-0">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className="text-sm font-semibold text-dash-ink2 truncate group-hover:text-teal-600 transition">
                          ...
                          <p className="text-xs text-dash-mute truncate mb-2">Batch: {batch.id}</p>
                          <div className="flex flex-wrap items-center gap-3 text-[11px] text-dash-mute2">
                            <span className="flex items-center gap-1">
                              <FiCalendar size={11} className="text-dash-faint" />
                              {formatDate(batch.startDate)} — {formatDate(batch.endDate)}
                            </span>
                            <span className="flex items-center gap-1">
                              <FiUsers size={11} className="text-dash-faint" />
                              <span className="font-semibold text-dash-ink4">{batch.studentCount || 0}</span> students
                            </span>
                            {batch.mentorId?.name && (
                              <span className="flex items-center gap-1">
                                <FiUserCheck size={11} className="text-dash-faint" />
                                {batch.mentorId.name}
                              </span>
                            )}
                          </div>
                        ...
                        <div className="sm:w-[120px] shrink-0">
                          <div className="flex items-center justify-between mb-1.5">
                            <span className="text-[10px] text-dash-mute2 font-medium">Certified</span>
                            <span className={`text-[11px] font-bold ${seatPct >= 80 ? 'text-emerald-600' : seatPct >= 50 ? 'text-amber-600' : 'text-dash-mute'}`}>
                              {seatPct}%
                            </span>
                          </div>
                          <div className="w-full bg-dash-soft2 rounded-full h-2">
                          ...
                          <p className="text-[10px] text-dash-mute2 mt-1 text-center">{batch.certifiedCount || 0}/{batch.studentCount || 0}</p>
                        </div>
                        <FiChevronRight className="text-dash-faint group-hover:text-teal-500 group-hover:translate-x-1 transition-all shrink-0" size={18} />
                      </div>
                    </div>
                  );
                })}
  ...
  return (
    <div className="poppins space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 bg-dash-card rounded-xl border border-dash-line/60 p-4 shadow-sm">
        <div className="flex items-center gap-3">
          <button onClick={goBack} className="w-9 h-9 rounded-lg bg-dash-soft2 flex items-center justify-center text-dash-mute hover:bg-teal-50 hover:text-teal-600 transition shrink-0">
            <FiArrowLeft size={16} />
          </button>
          <div>
            <h1 className="text-lg font-bold text-dash-ink outfit">{batchInfo?.courseName || selectedBatch?.courseName || 'Batch'}</h1>
            <div className="flex flex-wrap items-center gap-2 mt-0.5 text-[11px] text-dash-mute2">
        ...
        <div className="flex items-center gap-2">
          {!bulkMode ? (
            ...
            <>
              ...
              <button onClick={() => { setBulkMode(false); setSelectedStudents([]); }} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-dash-ink4 bg-dash-soft border border-dash-line rounded-lg hover:bg-dash-soft2 transition">
                Cancel
              </button>
            </>
          )}
      ...
      <div className="flex flex-col sm:flex-row gap-3">
        ...
        {bulkMode && (
          <button onClick={toggleSelectAll} className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-dash-ink4 bg-dash-soft border border-dash-line rounded-lg hover:bg-dash-soft2 transition">
        ...
        <span className="text-sm text-dash-mute2 self-center">{filteredStudents.length} of {students.length} students</span>
      </div>
      ...
      {loading ? (
        <div className="bg-dash-card rounded-xl border border-dash-line/60 shadow-sm overflow-hidden">
          {[1, 2, 3, 4, 5].map(i => (
            <div key={i} className="h-[56px] border-b border-dash-soft bg-dash-soft/50 animate-pulse" />
          ))}
        </div>
        ...
        <div className="bg-dash-card rounded-2xl border border-dash-line/60 p-16 text-center shadow-sm">
          ...
          <h3 className="font-medium text-dash-ink4 text-base">No Students Found</h3>
          <p className="text-sm text-dash-mute2 mt-1">No enrolled students in this batch.</p>
        </div>
      ) : (
        <div className="bg-dash-card rounded-xl border border-dash-line/60 shadow-sm overflow-hidden">
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-dash-soft/80 border-b border-dash-line-soft">
                  ...
                  <th className="text-left px-4 py-3 text-[11px] font-semibold text-dash-mute">Student</th>
                  <th className="text-left px-3 py-3 text-[11px] font-semibold text-dash-mute">Contact</th>
                  <th className="text-center px-3 py-3 text-[11px] font-semibold text-dash-mute">Payment</th>
                  <th className="text-center px-3 py-3 text-[11px] font-semibold text-dash-mute">Attendance</th>
                  <th className="text-center px-3 py-3 text-[11px] font-semibold text-dash-mute">Exam</th>
                  <th className="text-center px-3 py-3 text-[11px] font-semibold text-dash-mute">Certificate</th>
                  <th className="text-center px-3 py-3 text-[11px] font-semibold text-dash-mute">Action</th>
                </tr>
              </thead>
              <tbody>
                {filteredStudents.map((student) => {
                  ...
                  const certBadgeCls = isCertified ? 'bg-emerald-50 text-emerald-600 border-emerald-200'
                    ...
                    : 'bg-dash-soft text-dash-mute2 border-dash-line';
                  ...
                  return (
                    <tr key={student.studentId} className={`border-b border-dash-line-soft hover:bg-dash-soft/40 transition ${isSelected ? 'bg-violet-50/50' : ''}`}>
                      {/* Checkbox */}
                      {bulkMode && (
                        <td className="px-3 py-3">
                          {(!isCertified || isRevoked) ? (
                            <button onClick={() => setSelectedStudents(prev => prev.includes(student.studentId) ? prev.filter(id => id !== student.studentId) : [...prev, student.studentId])}
                              className={`w-4 h-4 rounded border-2 flex items-center justify-center transition ${isSelected ? 'bg-violet-500 border-violet-500 text-white' : 'border-dash-line-strong hover:border-violet-400'}`}>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2.5">
                          ...
                          <div className="min-w-0">
                            <p className="text-xs font-semibold text-dash-ink3 leading-tight truncate max-w-[140px]">{student.studentName}</p>
                            <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded-full ${student.enrollmentStatus === 'active' ? 'bg-emerald-50 text-emerald-600' : student.enrollmentStatus === 'completed' ? 'bg-blue-50 text-blue-600' : 'bg-dash-soft2 text-dash-mute'}`}>
                      ...
                      {/* Contact */}
                      <td className="px-3 py-3">
                        <p className="text-[11px] text-dash-ink4 truncate max-w-[160px]">{student.email || '—'}</p>
                        <p className="text-[10px] text-dash-mute2 flex items-center gap-1 mt-0.5">
                      ...
                      <td className="px-3 py-3 text-center">
                        ...
                        <p className="text-[10px] text-dash-mute2 mt-0.5">{formatTaka(pay.amount)}</p>
                        {pay.installments?.total > 0 && (
                          <p className="text-[9px] text-dash-mute2">
                      ...
                      <td className="px-3 py-3 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          <div className="w-12 bg-dash-soft2 rounded-full h-1.5">
                        ...
                        <p className="text-[9px] text-dash-mute2 mt-0.5">
                      ...
                      <td className="px-3 py-3 text-center">
                        {exam.totalExams > 0 ? (
                          ...
                          <span className="text-[10px] text-dash-mute2">No exams</span>
                        )}
                      </td>
                      ...
                      <td className="px-3 py-3 text-center">
                        <div className="flex items-center justify-center gap-1">
                          ...
                          <button onClick={() => toggleEligibility(student)} disabled={isLoading}
                            className={`inline-flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium rounded-lg border transition disabled:opacity-50 ${isLoading ? 'bg-dash-soft text-dash-mute2 border-dash-line' : isCertified ? 'text-red-500 bg-red-50 hover:bg-red-100 border-red-200' : 'text-white bg-emerald-600 hover:bg-emerald-700 border-emerald-600 shadow-sm'}`}>
