  ...
  return (
    <div className="space-y-6 poppins">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-dash-ink outfit">Partners / Collaborations</h1>
          <p className="text-sm text-dash-mute mt-1">হোমপেজের চলমান logo carousel — যেকোনো ওয়েবসাইটের logo + link যোগ করুন।</p>
        </div>
      ...
      {loading ? (
        ...
        <div className="bg-dash-card rounded-2xl border border-dashed border-dash-line p-16 text-center">
          <div className="w-16 h-16 rounded-2xl bg-brand-soft flex items-center justify-center mx-auto mb-4"><FiUsers className="text-brand" size={26} /></div>
          <h3 className="text-base font-bold text-dash-ink3">No partners yet</h3>
          <p className="text-sm text-dash-mute2 mt-1">প্রথম partner/website যোগ করুন।</p>
          <button onClick={openNew} className="mt-5 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-brand text-white text-sm font-bold"><FiPlus size={14} /> New Partner</button>
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {list.map((p) => (
            <div key={p._id} className={`bg-dash-card rounded-xl border border-dash-line shadow-sm p-4 ${p.isActive === false ? 'opacity-60' : ''}`}>
              <div className="h-16 flex items-center justify-center bg-dash-soft rounded-lg mb-3 px-3">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img src={p.logoUrl} alt={p.name} className="max-h-10 w-auto object-contain" />
              </div>
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <p className="font-bold text-dash-ink2 text-sm truncate">{p.name}</p>
                  {p.websiteUrl && (
                    <a href={p.websiteUrl} target="_blank" rel="noreferrer" className="inline-flex items-center gap-1 text-[11px] text-brand-ink hover:underline truncate max-w-[160px]">
                      <FiExternalLink size={10} /> {p.websiteUrl.replace(/^https?:\/\//, '')}
                    </a>
                  )}
                  <p className="text-[10px] text-dash-mute2 mt-0.5">Order: {p.order ?? 0}</p>
                </div>
                <div className="flex items-center gap-1 shrink-0">
                  <button onClick={() => toggleActive(p)} title={p.isActive === false ? 'Show' : 'Hide'} className="w-8 h-8 rounded-lg flex items-center justify-center text-dash-mute2 hover:text-brand-ink hover:bg-brand-soft transition">
                    {p.isActive === false ? <FiEyeOff size={15} /> : <FiEye size={15} />}
                  </button>
                  <button onClick={() => openEdit(p)} title="Edit" className="w-8 h-8 rounded-lg flex items-center justify-center text-dash-mute2 hover:text-brand-ink hover:bg-brand-soft transition"><FiEdit3 size={15} /></button>
                  <button onClick={() => del(p)} title="Delete" className="w-8 h-8 rounded-lg flex items-center justify-center text-dash-mute2 hover:text-rose-500 hover:bg-rose-50 transition"><FiTrash2 size={15} /></button>
                </div>
              </div>
            </div>
          ))}
      ...
      {form && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm" onClick={() => !saving && setForm(null)}>
          <div className="bg-dash-card rounded-2xl shadow-2xl w-full max-w-md max-h-[92vh] overflow-hidden flex flex-col" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-4 overflow-y-auto">
              ...
              <div>
                ...
                {form.logoUrl ? (
                  <div className="flex items-center justify-between rounded-lg border border-brand-line bg-brand-soft/50 px-3 py-2.5">
                    ...
                    <button onClick={() => F('logoUrl', '')} className="text-dash-mute2 hover:text-rose-500 shrink-0"><FiX size={16} /></button>
                  </div>
                ) : (
              ...
              <div className="grid grid-cols-2 gap-3 items-end">
                ...
                <label className="flex items-center gap-2 text-sm text-dash-ink4 cursor-pointer pb-2"><input type="checkbox" checked={form.isActive} onChange={(e) => F('isActive', e.target.checked)} className="accent-brand" /> Active</label>
              </div>
            </div>
