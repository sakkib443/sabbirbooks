...
export default function MentorStudentProgressPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-dash-ink2 outfit">Student Progress</h1>
        <p className="text-sm text-dash-mute mt-0.5">প্রতি student-এর assignment নম্বর ও overall — admin ও student নিজেও দেখতে পারে</p>
      </div>
          ...
          {batches.map(batch => {
            ...
            return (
              <div key={batch._id} className="bg-dash-card rounded-xl border border-dash-line overflow-hidden hover:shadow-md transition-all">
                <button onClick={() => toggleBatch(batch._id)} className="w-full text-left px-5 py-4 hover:bg-brand-soft/40 transition">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-4">
                      <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-brand to-brand-hover flex items-center justify-center text-white shadow-sm">
                        <FiBook size={18} />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <h3 className="text-base font-bold text-dash-ink2">{batch.name || batch.courseName}</h3>
                          <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${batch.status === 'active' ? 'bg-emerald-50 text-emerald-600' : batch.status === 'upcoming' ? 'bg-blue-50 text-blue-600' : 'bg-dash-soft2 text-dash-mute'}`}>{batch.status}</span>
                          <code className="text-[10px] text-dash-mute2 bg-dash-soft px-1.5 py-0.5 rounded">{batch.id}</code>
                        </div>
                        <p className="text-xs text-dash-mute mt-0.5 flex items-center gap-1"><FiCalendar size={10} />{batch.classTime || '—'}</p>
                      </div>
                    </div>
                ...
                {isExpanded && (
                  <div className="border-t border-dash-line-soft bg-dash-soft/30">
                    {loadingProg ? (
                      <div className="flex items-center justify-center py-12"><FiLoader className="animate-spin text-brand" size={22} /><span className="ml-2 text-sm text-dash-mute2">Loading...</span></div>
                    ) : progress.students.length === 0 ? (
                      <div className="text-center py-12 text-sm text-dash-mute2"><FiUsers className="mx-auto mb-2" size={28} />No students enrolled</div>
                    ) : (
                      <div className="p-4">
                        <div className="flex items-center justify-between mb-3">
                          <span className="text-sm font-semibold text-dash-ink4">
                            {students.length} Students · <span className="text-brand-ink">{progress.assignments.length} Assignments</span>
                          </span>
                          <div className="flex items-center gap-1.5 bg-dash-card border border-dash-line rounded-lg px-2.5 py-1.5 w-52">
                            ...
                            <input value={searchTerm} onChange={e => setSearchTerm(e.target.value)} placeholder="Search students..." className="text-sm text-dash-ink4 outline-none bg-transparent w-full placeholder:text-dash-mute2" />
                        ...
                        {progress.assignments.length === 0 ? (
                          <div className="bg-dash-card rounded-xl border border-dashed border-dash-line p-8 text-center text-sm text-dash-mute2">
                            <FiClipboard className="mx-auto mb-2 text-dash-faint" size={26} />
                        ...
                        <div className="flex items-center gap-4 mt-3 text-[11px] text-dash-mute2">
