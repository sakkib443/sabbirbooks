...
const MentorEditCourse = () => {
    ...
    const inputClass = "w-full px-4 py-2 rounded-lg border border-dash-line focus:ring-2 focus:ring-aqua/20 focus:border-aqua outline-none text-[13px] bg-dash-card transition-all";
    const labelClass = "block text-[11px] font-bold text-dash-mute mb-1 uppercase tracking-widest";
    ...
    if (fetching) return (
        <div className="h-screen flex items-center justify-center bg-dash-card">
            <FiLoader className="animate-spin text-aqua" size={35} />
            <p className="ml-2 text-sm text-dash-mute font-medium uppercase tracking-widest">Loading Course Data...</p>
        </div>
    );

    return (
        <div className="min-h-screen bg-dash-soft p-4 md:p-8 font-poppins text-dash-ink3">
            <div className="max-w-5xl mx-auto pb-20">

                <div className="flex items-center justify-between mb-8">
                    <div>
                        <Link href="/dashboard/mentor/course" className="text-dash-mute flex items-center gap-2 text-xs font-bold hover:text-aqua transition-all">
                            <FiArrowLeft /> Back to Courses
                        </Link>
                        <h1 className="text-2xl font-black text-dash-ink mt-2 outfit">Edit Course</h1>
                    </div>
                    <button
                        onClick={handleSubmit(onSubmit)}
                        disabled={loading}
                        className="bg-aqua text-white px-8 py-2.5 rounded-xl font-bold text-sm shadow-lg hover:bg-aqua-hover disabled:bg-dash-faint transition-all flex items-center gap-2"
                    >
                ...
                <form className="space-y-6">
                    {/* Card 1 - Identity */}
                    <div className="bg-dash-card p-6 rounded-2xl border border-dash-line-soft shadow-sm grid grid-cols-1 md:grid-cols-3 gap-5">
                        <h2 className="md:col-span-3 text-[10px] font-black uppercase border-b pb-2 tracking-[2px] text-aqua">1. Identity & Classification</h2>
                    ...
                    {/* Card 2 - Metrics */}
                    <div className="bg-dash-card p-6 rounded-2xl border border-dash-line-soft shadow-sm grid grid-cols-2 md:grid-cols-4 gap-5">
                        <h2 className="md:col-span-4 text-[10px] font-black uppercase border-b pb-2 tracking-[2px] text-aqua">2. Metrics</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="bg-dash-card p-6 rounded-2xl border shadow-sm">
                            <div className="flex justify-between items-center mb-4 border-b pb-2">
                                <label className="text-[10px] font-black uppercase tracking-widest text-aqua">Curriculum</label>
                                <button type="button" onClick={() => curriculums.append('')} className="text-[10px] font-black text-dash-steel underline">+ ADD MODULE</button>
                            </div>
                    ...
                    <div className="bg-dash-card p-6 rounded-2xl border shadow-sm space-y-4">
                        <h2 className="text-[10px] font-black uppercase border-b pb-2 tracking-[2px] text-aqua">3. Narratives</h2>
