...
const typeBadge = (t) => ({
  ...
  offline: { label: 'Offline', cls: 'bg-[#FEF6E7] text-[#a5680f] border-[#F0DFB4]', icon: <FiMapPin size={10} /> },
  recorded: { label: 'Recorded', cls: 'bg-slate-100 text-slate-600 border-slate-200', icon: <FiVideo size={10} /> },
}[t] || { label: t || 'Class', cls: 'bg-slate-100 text-slate-600 border-slate-200', icon: <FiFolder size={10} /> });
...
export default function MentorSchedulePage() {
  ...
  const ClassRow = ({ cls }) => {
    ...
    return (
      <div className="flex items-stretch gap-3 bg-white rounded-xl border border-slate-200 hover:shadow-md hover:border-[#F0DFB4] transition overflow-hidden">
        {/* date block */}
        <div className={`flex flex-col items-center justify-center px-4 py-3 shrink-0 w-[68px] ${overdue ? 'bg-rose-50 text-rose-600' : 'bg-[#FEF6E7] text-[#a5680f]'}`}>
          ...
          <span className={`text-[9px] ${overdue ? 'text-rose-400' : 'text-[#c9871a]'}`}>{d ? d.toLocaleDateString('en-GB', { weekday: 'short' }) : ''}</span>
        </div>
        {/* details */}
        <div className="flex-1 min-w-0 py-3 pr-3">
          <div className="flex items-center gap-2 flex-wrap">
            <h3 className="text-sm font-bold text-slate-800 truncate">{cls.title || cls.topic || 'Class'}</h3>
          ...
          {cls.topic && <p className="text-xs text-slate-500 truncate mt-0.5">{cls.topic}</p>}
          <div className="flex items-center gap-3 mt-1.5 text-[11px] text-slate-400 flex-wrap">
            <span className="flex items-center gap-1"><FiClock size={10} className="text-[#c9871a]" /> {cls.startTime || '—'}{cls.endTime ? `–${cls.endTime}` : ''}</span>
        ...
        <div className="flex flex-col items-end justify-center gap-1.5 pr-3 py-3 shrink-0">
          ...
          <div className="flex items-center gap-2">
            {cls.type === 'live' && cls.meetingLink && (
              <a href={cls.meetingLink} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1 text-[11px] font-bold text-[#F3A522] hover:underline">Join <FiExternalLink size={10} /></a>
            )}
            {bId && (
              <Link href={`/dashboard/mentor/batch-materials/${bId}`} className="inline-flex items-center gap-1 text-[11px] font-semibold text-[#c9871a] hover:underline">Manage <FiArrowRight size={10} /></Link>
            )}
          </div>
        </div>
      </div>
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-0.5">তোমার <b className="text-[#c9871a]">পরবর্তী ক্লাসগুলো</b> · Complete করলে সরে যাবে · তৈরি/এডিট হয় <b className="text-[#c9871a]">Batch Materials</b> থেকে</p>
        </div>
      ...
      <div className="grid grid-cols-3 gap-4">
        {[
          { label: 'Next Classes', value: nextClasses.length, c: 'text-[#c9871a]', b: 'bg-[#FEF6E7]' },
          ...
          <div key={s.label} className={`rounded-xl border border-slate-200/70 p-4 ${s.b}`}>
      ...
      {nextClasses.length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-16 text-center">
          ...
          <h3 className="text-lg font-bold text-slate-700 mb-1">No Upcoming Classes</h3>
          <p className="text-sm text-slate-500 mb-4">সব ক্লাস completed অথবা এখনো তৈরি হয়নি। নতুন ক্লাস Batch Materials থেকে তৈরি করো।</p>
          <Link href="/dashboard/mentor/batch-materials" className="inline-flex items-center gap-1.5 px-4 py-2 rounded-lg bg-[#F3A522] text-white text-sm font-bold hover:bg-[#e0941c] transition">
      ...
      {completedCount > 0 && (
        <p className="text-center text-xs text-slate-400">
          {completedCount}টি ক্লাস completed — history দেখতে <Link href="/dashboard/mentor/batch-materials" className="text-[#c9871a] font-semibold hover:underline">Batch Materials</Link>-এ যাও
        </p>
      )}
